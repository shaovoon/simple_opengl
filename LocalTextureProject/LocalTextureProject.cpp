...
#define IMG_FILE2 "joeyswee.jpg"

struct UserData
{
	UserData() : programObject(0), wmpLoc(-1), positionLoc(-1), texCoordLoc(-1), samplerLoc(-1), 
		textureId(0), textureId2(0), images_loaded(0) {}
...
extern "C" {
	...
	void load_2nd_texture_from_memory(unsigned char* bytes, int size);
}

GLuint init_texture_from_memory(unsigned char* bytes, int size);

GLuint init_texture_from_memory_wrapper(const char* file); // for testing before Emscripten use
...
int main()
{
	...
	else
	{
		...
		if(gUserData.textureId2)
			deinit_texture(gUserData.textureId2);
	}
...
{
#ifdef __EMSCRIPTEN__
	emscripten_async_wget("https://localhost:44303/yes.png", IMG_FILE, load_texture, load_error);
#endif
...
void render()
{
	...
	if(gUserData.textureId2)
		glBindTexture(GL_TEXTURE_2D, gUserData.textureId2);
	else
...
GLuint init_texture_from_memory(unsigned char* bytes, int size) {
	GLuint texture = 0;
	SDL_RWops* rw_ops = SDL_RWFromMem((void*)bytes, size);
...
GLuint init_jpeg_texture_from_memory(unsigned char* bytes, int size) {
	GLuint texture = 0;
	int width, height, channels;
	//stbi_set_flip_vertically_on_load(true);
	unsigned char* image = stbi_load_from_memory(bytes, size,
		&width,
		&height,
		&channels,
		STBI_rgb);
	if (image == NULL) { //If it failed, say why and don't continue loading the texture
		printf("Error: \"%s\"\n", SDL_GetError()); return 0;
	}
...
extern "C" {
	void load_2nd_texture_from_memory(unsigned char* bytes, int size)
	{
		//gUserData.textureId2 = init_texture_from_memory(bytes, size);
		...
		{
			++pos;
			char* charbytes = (char*)(bytes);
			std::string str = charbytes;

			std::string short_str = str.substr(0, 50);
			std::string decoded = base64_decode(charbytes + pos);
			if (short_str.find("image/jpeg") != std::string::npos) // is jpeg
			{
				gUserData.textureId2 = init_jpeg_texture_from_memory((unsigned char*)decoded.data(), decoded.size());
			}
			else
			{
				gUserData.textureId2 = init_texture_from_memory((unsigned char*)decoded.data(), decoded.size());
			}
		}
	}
}

GLuint init_texture_from_memory_wrapper(const char* file)
{
	using namespace std;
	FILE* fp = nullptr;
#ifdef _WIN32
	...
	return init_jpeg_texture_from_memory(vec.data(), size);
}
